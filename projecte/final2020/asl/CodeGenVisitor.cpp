...
antlrcpp::Any CodeGenVisitor::visitSum(AslParser::SumContext *ctx) {
  ...
  TypesMgr::TypeId valueType = getTypeDecor(ctx);
  
  
  
  if(Types.isFloatTy(valueType)) {
    code = instruction::FLOAD(tempValue,"0.0");
    
    for(auto expr : ctx->expr()) {
      CodeAttribs  && aux = visit(expr);
      code = code || aux.code;
      
      TypesMgr::TypeId exprType = getTypeDecor(expr);
      std::string temp = "%"+codeCounters.newTEMP();
      if(not Types.isFloatTy(exprType)) {
        code = code || instruction::FLOAT(temp, aux.addr);
      }
      else temp = aux.addr;
      
      code = code || instruction::FADD(tempValue,tempValue,temp);
    }
  }
  else {
    code = instruction::ILOAD(tempValue,"0");
    
    for(auto expr : ctx->expr()) {
      CodeAttribs  && aux = visit(expr);
      code = code
        || aux.code
        || instruction::ADD(tempValue,tempValue,aux.addr);
    }
  }

  CodeAttribs codAts(tempValue, "", code);
  DEBUG_EXIT();
  return codAts;
}
...
antlrcpp::Any CodeGenVisitor::visitFilter(AslParser::FilterContext *ctx) {
  ...
  CodeAttribs     && c1 = visit(ctx->ident(0));
  CodeAttribs     && c2 = visit(ctx->ident(1));
  CodeAttribs     && c3 = visit(ctx->ident(2));
  
  TypesMgr::TypeId t = getTypeDecor(ctx->ident(0));
  ...
  std::string labelW = codeCounters.newLabelWHILE();
  std::string labelBeginFilter = "beginFilter"+labelW;
  std::string labelEndFilter = "endFilter"+labelW;
  
  std::string labelL = codeCounters.newLabelIF();
  std::string labelEndIf = "endIf"+labelL;
  
  code = code
    || instruction::ILOAD(tempR,"0")
    || instruction::ILOAD(tempINCR,"1")
    || instruction::ILOAD(tempI,"0")
    || instruction::ILOAD(tempL,std::to_string(Types.getArraySize(t)))
    
    || instruction::LABEL(labelBeginFilter)
    || instruction::LT(tempC,tempI,tempL)
    || instruction::FJUMP(tempC,labelEndFilter)
    
    || instruction::LOADX(tempIN,c1.addr,tempI)
    || instruction::PUSH()
    || instruction::PUSH(tempIN)
    
    || instruction::CALL(c3.addr)
    
    || instruction::POP()
    || instruction::POP(tempOUT)
    
    || instruction::XLOAD(c2.addr,tempI,tempOUT)
    || instruction::FJUMP(tempOUT,labelEndIf)
    || instruction::ADD(tempR,tempR,tempINCR)
    || instruction::LABEL(labelEndIf)
    
    || instruction::ADD(tempI,tempI,tempINCR)
    || instruction::UJUMP(labelBeginFilter)
    || instruction::LABEL(labelEndFilter)
    ;
  
  CodeAttribs codAts(tempR, "", code);
  DEBUG_EXIT();
  return codAts;
}
