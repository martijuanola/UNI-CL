...
void SemErrors::packUnpackWithNonArray(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Pack/unpack with non array operand.");
  ErrorList.push_back(error);
}

void SemErrors::packUnpackNumberOfItemsMismatch(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "The number of items mismatch wrt. array size in pack/unpack.");
  ErrorList.push_back(error);
}

void SemErrors::packUnpackIncompatibleTypes(antlr4::ParserRuleContext *ctx,
					    unsigned int n) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Item #" + std::to_string(n) + " with incompatible types in pack/unpack operation.");
  ErrorList.push_back(error);
}

void SemErrors::nonTupleInTupleAccess(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Tuple access to a non tuple operand.");
  ErrorList.push_back(error);
}

void SemErrors::nonExistentFieldInTuple(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Tuple access to a non existent field number.");
  ErrorList.push_back(error);
}
