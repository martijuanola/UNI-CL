...
antlrcpp::Any TypeCheckVisitor::visitFor(AslParser::ForContext *ctx) {
	DEBUG_ENTER();
	...
	for(auto expr : ctx->expr()) {
		...
		if((not Types.isErrorTy(t1)) and (not Types.isIntegerTy(t1))) {
			Errors.forRequireIntegerExpr(expr);
		}
	}
...
antlrcpp::Any TypeCheckVisitor::visitMax(AslParser::MaxContext *ctx) {
	...
	for(auto expr : ctx->expr()) {
		visit(expr);
		TypesMgr::TypeId t1 = getTypeDecor(expr);
		if(Types.isCharacterTy(t1)) charfound = true;
		else if(Types.isNumericTy(t1)) {
			numericfound = true;
			if(Types.isFloatTy(t1)) floatfound = true;
		}
	}
	
	if(numericfound and charfound) {
	...
	if(size < 2) Errors.numberOfMaxArguments(ctx);
