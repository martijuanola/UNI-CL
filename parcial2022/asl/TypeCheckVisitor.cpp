...
antlrcpp::Any TypeCheckVisitor::visitMapeig(AslParser::MapeigContext *ctx) {
  DEBUG_ENTER();
  
  //visitar totes les expressions i idents
  ...
  if((not Types.isErrorTy(a) and not Types.isArrayTy(a)) or 
	(not Types.isErrorTy(b) and not Types.isArrayTy(b))) {
		Errors.mapWithNonArraysOrDifferentSizes(ctx);
  }
  else if(Types.getArraySize(a) != Types.getArraySize(b)) {
	  Errors.mapWithNonArraysOrDifferentSizes(ctx);
  }
  ...
  if(Types.isArrayTy(a)) {
	  TypesMgr::TypeId aelem = Types.getArrayElemType(a);
	  TypesMgr::TypeId expr1 = getTypeDecor(ctx->expr()[1]);
	  TypesMgr::TypeId expr2 = getTypeDecor(ctx->expr()[2]);
	  if(not Types.isErrorTy(aelem) and (
	  (not Types.isErrorTy(expr1) and not Types.equalTypes(aelem, expr1)) or 
	  (not Types.isErrorTy(expr2) and not Types.equalTypes(aelem, expr2)))) {
		  if(not (Types.isFloatTy(aelem) and (Types.isNumericTy(expr1) or Types.isNumericTy(expr2)))) {
